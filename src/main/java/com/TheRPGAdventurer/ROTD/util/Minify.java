...
//
//  Strips comments, whitespace, line markers out of json before parsing.
// minifyCommentsOnly - remove comments only - makes parsing error identification easier if whitespace and lines not removed
...
public abstract class Minify {
  ...
  public static String minifyCommentsOnly(CharSequence json) {
    if (json == null)
      throw new IllegalArgumentException("Parameter 'json' cannot be null.");
...
//          tmp2 = tmp2.replaceAll("(\\n|\\r|\\s)*", "");
      ...
      if (tmp.charAt(0) == '\"' && !in_multiline_comment && !in_singleline_comment) {
        ...
        if (!in_multiline_comment && in_singleline_comment) in_singleline_comment = false;
        new_str.append(tmp);
      } else if (!in_multiline_comment && !in_singleline_comment) { // && !tmp.substring(0, 1).matches("\\n|\\r|\\s")) {
        new_str.append(tmp);
      }
